...
from services.serializers.lending import BorrowBookSerializer, LendingSerializer, ReturnBookSerializer
...
class BorrowBookController(ResponseMixin, APIView):
    def post(self, request):
        ...
        return self.success_response(data=LendingSerializer(lending).data, message="Book borrowed successfully.", status_code=status.HTTP_201_CREATED)
...
class ReturnBookController(ResponseMixin, APIView):
    def post(self, request, uuid):
        ...
        return self.success_response(data=LendingSerializer(lending).data, message="Book returned successfully.")
...
class LendingListController(ResponseMixin, APIView):
    def get(self, request):
        ...
        return self.paginated_response(serializer.data, page, page_size, queryset.count(), message="Lending records retrieved successfully.")


class LendingDetailController(ResponseMixin, APIView):
    def get(self, request, uuid):
        lending = get_object_or_404(Lending, uuid=uuid, deleted_at__isnull=True)
        return self.success_response(data=LendingSerializer(lending).data, message="Lending retrieved successfully.")
...
class MemberBorrowedBooksController(ResponseMixin, APIView):
    def get(self, request, member_uuid):
        ...
        return self.success_response(data=LendingSerializer(lendings, many=True).data, message="Borrowed books retrieved successfully.")


class OverdueListController(ResponseMixin, APIView):
    def get(self, request):
        overdue = LendingFactory.get_overdue_records()
        return self.success_response(data=LendingSerializer(overdue, many=True).data, message="Overdue records retrieved successfully.")
